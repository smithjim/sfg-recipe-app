...
public class RecipeDataBoot implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public RecipeDataBoot(RecipeRepository recipeRepository, CategoryRepository categoryRepository, UnitOfMeasureRepository unitOfMeasureRepository) {
    ...
    private List<Recipe> buildRecipes() {
        ...
        //get UOMs
        UnitOfMeasure eachUom = fetchUnitOfMeasure("Each");
        UnitOfMeasure tableSpoonUom = fetchUnitOfMeasure("Tablespoon");
        UnitOfMeasure teaspoonUom = fetchUnitOfMeasure("Teaspoon");
        UnitOfMeasure dashUom = fetchUnitOfMeasure("Dash");
        UnitOfMeasure pintUom = fetchUnitOfMeasure("Pint");
        UnitOfMeasure cupsUom = fetchUnitOfMeasure("Cup");
    ...
    private UnitOfMeasure fetchUnitOfMeasure(String description) {
        Optional<UnitOfMeasure> uomOption = unitOfMeasureRepository.findByDescription(description);

        if(! uomOption.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found for: " + description);
        }

        return uomOption.get();
    }

    private Category fetchCategoryFromDescription(String description) {
        //get Categories
        Optional<Category> categoryOptional = categoryRepository.findByDescription(description);

        if(! categoryOptional.isPresent()){
            throw new RuntimeException("Expected Category Not Found : " + description);
        }
